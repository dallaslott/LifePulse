...
async function refreshBea(data, notes) {
  ...
  const payload = await fetchJson(`https://apps.bea.gov/api/data?${params}`, 120000);
  const rows = payload?.BEAAPI?.Results?.Data;
  if (!Array.isArray(rows) || !rows.length) throw new Error('BEA county-income response was empty');
  const counties = {};
  let latestYear = 0;
  for (const row of rows) {
    const geoName = String(row.GeoName || '').replace(/\s*\*+$/, '');
    const [rawCounty, stateAbbr] = geoName.split(',').map(item => item.trim());
    const stateFips = String(row.GeoFIPS || '').replace(/\D/g, '').slice(0, 2);
    ...
    const entry = counties[key] || { name: rawCounty, state, stateFips, countyFips: String(row.GeoFIPS || '').replace(/\D/g, '').slice(0, 5), series: {} };
